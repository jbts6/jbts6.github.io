function hex_password(pwd, key) {
    var hexone = sha512.hmac(key, pwd),
        hextwo = sha512.hmac("hello", hexone),
        hexthree = sha512.hmac("world", hexone),
        source = hextwo.split(""),
        rule = hexthree.split("");
...
function seek_password(hash, length, rule_of_punctuation, rule_of_letter) {
    var lower = "abcdefghijklmnopqrstuvwxyz".split(""),
        ...
        punctuation = ",.:;!?".split(""),
        alphabet = lower.concat(number);
    1 == parseInt(rule_of_punctuation) &&
        (alphabet = alphabet.concat(punctuation)),
        1 == parseInt(rule_of_letter) && (alphabet = alphabet.concat(upper));
    for (var i = 0; i <= hash.length - length; ++i) {
        var sub_hash = hash.slice(i, i + parseInt(length)).split(""),
            ...
            sk_pwd = sub_hash
                .map(function (c) {
                    return (count = (count + c.charCodeAt()) % alphabet.length);
                })
                .map(function (k) {
                    return alphabet[k];
                }),
        ...
        if (
            ...
            -1 == parseInt(rule_of_letter) && (matched[1] = !0),
            -1 == parseInt(rule_of_punctuation) && (matched[3] = !0),
            !matched.includes(!1))
        )
            return sk_pwd.join("");
    }
...
function generate_password() {
    var pwd = document.getElementById("pwd").value,
        ...
        rule_of_punctuation = get_select_option("rule_of_punctuation"),
        rule_of_letter = get_select_option("rule_of_letter"),
        pwd_length = document.getElementById("pwd_length").value;
    if (pwd && key) {
        var hash = hex_password(pwd, key),
            sk_pwd;
        return (
            console.assert(128 === hash.length, "hash长度不是128位！"),
            seek_password(
                hash,
                pwd_length,

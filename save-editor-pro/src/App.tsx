...
import { decodeSaveText, encodeSaveText, type SaveKind, type SaveTextParts } from "./codec";
...
function App() {
  ...
  const handleLoadSave = async (event: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      const decoded = await decodeSaveText(text, file.name);
      setSaveState({
        value: decoded.value,
        parts: decoded.parts,
        sourceName: file.name,
        kind: decoded.kind,
        saveId: decoded.saveId,
      });
      setSaveOutput("");
      setUndoStack([]);
      setError(null);
  ...
  const buildSaveOutput = async (): Promise<void> => {
    if (!saveState) {
      setError("请先加载并解密存档。");
      return;
    }
    try {
      const next = await encodeSaveText(
        saveState.value,
        saveState.kind,
        saveState.saveId,
        preserveAffix ? saveState.parts : undefined
      );
      setSaveOutput(next);
      setError(null);
